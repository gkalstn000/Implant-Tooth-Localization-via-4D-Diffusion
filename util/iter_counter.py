...
class IterationCounter():
    def __init__(self, opt, dataset_size):
        ...
        self.batchSize = opt.data.train.batch_size

        self.first_epoch = 1
        self.total_epochs = opt.max_epoch
        self.epoch_iter = 0  # iter number within each epoch
        self.iter_record_path = os.path.join(self.opt.checkpoints_dir, self.opt.exp_name, 'iter.txt')
    ...
    def record_one_iteration(self):
        ...
        # so we can assume batch size is always batchSize
