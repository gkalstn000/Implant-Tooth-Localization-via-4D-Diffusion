...
class BeatGANsAutoencModel(BeatGANsUNetModel):
    def __init__(self, conf):
        ...
        conf.in_channels = 1
        self.encoder = EightLayerFC(input_size=3,
                                    hidden_size=conf.embed_channels,
                                    output_size=conf.embed_channels)
    ...
    def forward(self,
        ...
        if cond is None:
            x_cond  = (cond_mask.view(-1, 1)*x_cond)
            if x is not None:
        ...
        enc_time_emb = emb + cond
        mid_time_emb = emb + cond
        dec_time_emb = emb + cond
        ...
        if x is not None:
            ...
            # middle blocks
            h = self.middle_block(h, emb=mid_time_emb, cond=mid_cond_emb, start_frame_index=frame_start)
        else:
