...
from make_a_video_pytorch import SpatioTemporalAttention
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    """
    def forward(self, x, emb=None, cond=None, lateral=None, start_frame_index=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb=emb, cond=None, lateral=lateral)
            elif isinstance(layer, AttentionBlock) or isinstance(layer, SpatioTemporalAttention):
                x = layer(x, start_frame_index=start_frame_index)
            else:
                x = layer(x)
        return x
...
class SpatioTemporalAttentionWithAbsolutePosition(SpatioTemporalAttention):
    def __init__(self, *args, max_frames, sub_frame, resolution, **kwargs):
        super().__init__(*args, **kwargs)
        # 추가적인 시간적 위치 인코딩 임베딩을 생성
        self.temporal_pos_embedding = nn.Embedding(max_frames, resolution**2)
    ...
    def forward(self, x, start_frame_index, enable_time=True):
        ...
        # 프레임 인덱스가 최대 프레임 수를 초과하지 않도록 검사
        sub_frame_indices = th.clamp(sub_frame_indices, 0, self.max_frames - 1)

        # 프레임 인덱스에 해당하는 포지셔널 인코딩을 얻습니다.
        temporal_pos = self.temporal_pos_embedding(sub_frame_indices).view(b, f, -1)

        # 입력 x에 맞게 차원을 조정합니다.
        temporal_pos = temporal_pos.unsqueeze(1).unsqueeze(-2).view(b, 1, -1, h, w)

        # x에 시간적 위치를 더합니다.
        x = x + temporal_pos

        # 부모 클래스의 forward 메서드를 호출합니다.
        return super().forward(x, enable_time=enable_time)